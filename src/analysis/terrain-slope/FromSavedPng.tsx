...
import { getCanvasObject } from "./storage.ts";
...
export default function FromSavedPng({ steepnessThreshold }: Props) {
  ...
  useEffect(() => {
    getCanvasObject("terrain-steepness-png").then((res) => {
      if (!res) {
        return;
      }
  ...
  return (
    <weave-button onClick={displayPng}>Draw slope from stored png</weave-button>
  );
}
